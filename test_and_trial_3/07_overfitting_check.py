#!/usr/bin/env python3
"""
Step 7: Overfitting / plagiarism / repetition detection.
...
3. Self-repetition: how much a generated piece repeats its own patterns
4. Per-file similarity scores
"""
...
def self_repetition_score(pitches, n=4):
    """What fraction of n-grams in a sequence appear more than once.

    High values indicate the piece is internally repetitive.
    """
    if len(pitches) < n + 1:
        return 0.0
...
def longest_repeated_substring_len(pitches):
    """Find the longest substring that appears at least twice in the sequence.

    Detects internal repetition / looping patterns.
    """
    if len(pitches) < 4:
        return 0
...
def main():
    ...
    for variant in ["constrained", "unconstrained"]:
        ...
        for f in sorted(os.listdir(var_dir)):
            ...
            flag = ""
            if max_lcs > 30 or coverage.get("pitch_8gram", 0) > 0.5:
                flag += " MEMORIZATION?"
            if longest_self_rep > 20 or self_rep.get("self_rep_4gram", 0) > 0.6:
                flag += " REPETITIVE?"
            print(f"  {f}: lcs={max_lcs}, 5gram={coverage.get('pitch_5gram', 0):.2f}, "
                  f"self_rep4={self_rep.get('self_rep_4gram', 0):.2f}, "
                  f"longest_self={longest_self_rep}{flag}")
        ...
        if file_results:
            ...
            avg_self_rep4 = np.mean([r["self_repetition"]["self_rep_4gram"] for r in file_results])
            avg_longest_self = np.mean([r["self_repetition"]["longest_self_repeat"] for r in file_results])
            results[variant] = {
                "files": file_results,
                "summary": {
                    "mean_pitch_5gram_coverage": round(avg_5gram, 4),
                    "mean_max_lcs": round(avg_lcs, 1),
                    "mean_self_rep_4gram": round(avg_self_rep4, 4),
                    "mean_longest_self_repeat": round(avg_longest_self, 1),
    ...
    # Print summary
    print(f"\n{'='*70}")
    print("OVERFITTING & REPETITION ANALYSIS SUMMARY")
    print(f"{'='*70}")

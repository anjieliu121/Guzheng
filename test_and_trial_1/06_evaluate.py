...
def main():
    ...
    # Print summary
    print("\n=== SUMMARY ===")
    print(f"{'Variant':<20} {'Files':<6} {'Penta%':<8} {'Density':<8} {'OA_PC':<8} {'OA_Dur':<8}")
    print("-" * 58)
    if train_agg:
        print(f"{'training':<20} {train_agg['n_files']:<6} {train_agg['mean_penta_purity']:<8} {train_agg['mean_density']:<8} {'1.0000':<8} {'1.0000':<8}")
    for name, v in variants.items():
        a, o = v["aggregate"], v["oa"]
        print(f"{name:<20} {a['n_files']:<6} {a['mean_penta_purity']:<8} {a['mean_density']:<8} {o.get('OA_pitch_class','N/A'):<8} {o.get('OA_duration','N/A'):<8}")

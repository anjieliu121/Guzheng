...
def allowed_token_ids_for_step(
    ...
    ids = list(
        range(
            cfg.velocity_offset,
            cfg.velocity_offset + cfg.num_velocity_bins,
        )
    )
...
class GuzhengTransformer(nn.Module):
    ...
    def generate(
        ...
        for _ in range(max_new_tokens):
            ...
            logits, _ = self(x)
            row = (logits[0] / max(temperature, 1e-8)).reshape(-1)
            ...
            elif allowed_pitch_token_ids is not None and cur_len >= prefix_len and (cur_len - prefix_len) % 4 == 1:
                allow = sorted(allowed_pitch_token_ids)
            else:

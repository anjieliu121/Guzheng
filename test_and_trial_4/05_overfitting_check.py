#!/usr/bin/env python3
"""
Step 5: Overfitting / plagiarism / repetition detection.
...
def load_training_sequences(train_dir):
    ...
    return pitch_seqs, interval_seqs


def analyze_generated_dir(var_dir, train_pitch_seqs, train_interval_seqs, train_dir):
    """Analyze a directory of generated MIDI files."""
    file_results = []
    ...
    for f in sorted(os.listdir(var_dir)):
        ...
        max_lcs = 0
        closest_train = ""
        train_files = sorted(tf for tf in os.listdir(train_dir) if tf.endswith(".mid"))
...
def main():
    ...
    train_pitch_seqs, train_interval_seqs = load_training_sequences(train_dir)
    ...
    test_pitch_seqs, _ = load_training_sequences(test_dir)

    results = {}

    # Analyze all generated checkpoint variants
        ...
        for ckpt_name in sorted(os.listdir(gen_dir)):
            ...
            for variant_type in ["constrained", "unconstrained"]:
                ...
                file_results = analyze_generated_dir(
                    var_dir, train_pitch_seqs, train_interval_seqs, train_dir
                )
                if file_results:
